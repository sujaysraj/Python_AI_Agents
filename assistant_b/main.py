...
messenger = Messenger(name="assistant_b", transport=transport, secret_key=SECRET_KEY)
...
def poll_for_incoming():
    ...
    while True:
        ...
        if incoming:
            print(f"[assistant_b] Got message from {incoming['sender']}: {incoming['message']}")
            if not incoming.get("user_initiated", False):
                print(f"[assistant_b] Skipping response — not user-initiated.")
                continue
...
while True:
    msg = input("[assistant_b] Type a message to send (or 'exit'): ").strip()
    if msg.lower() == "exit":
        break
    messenger.send(to="assistant_a", message=msg, user_initiated=True, msg_type="user")
